...
@section('content')
    <div class="page bg-white rounded pb-3">
        <div class="w-100 content-window">
            <div class="w-100 iransans p-3 mb-3 border-bottom d-flex flex-row align-items-center justify-content-between">
                <div>
                    <i class="fa fa-file-contract fa-1-4x ms-1"></i>
            ...
            <div class="page-header">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <button class="btn btn-outline-info ms-2" data-toggle="modal" data-target="#new_contract_modal">
                            <i class="fa fa-plus fa-1-2x ms-1"></i>
            ...
            <div id="table-scroll-container" class="p-3">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($contract_header->files)
                                        <a href="{{ route("ContractHeader.download",$contract_header->id) }}"><i
                                                    class="fa fa-download fa-1-6x info-color"></i></a>
                                    @else
                                ...
                                <td>
                                    <span class="iranyekan">{{ verta($contract_header->cretaed_at)->format("H:i:s Y/m/d") }}</span>
                                </td>
                                <td>
                                    <span class="iranyekan">{{ verta($contract_header->updated_at)->format("H:i:s Y/m/d") }}</span>
                                </td>
                                <td class="position-relative">
                                    <div class="dropdown table-functions iranyekan">
                                        <button class="btn btn-outline-light dropdown-toggle border-0 iranyekan info-color"
                                                type="button" id="dropdownMenuButton" data-toggle="dropdown"
                                                aria-haspopup="true" aria-expanded="false">
                                            <i class="fa fa-cog fa-1-4x"></i>
                                        </button>
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            {{--                                        @can("activation", "ContractHeader")--}}
                                            <form class="w-100" id="activation-form-{{ $contract_header->id }}"
                                                  action="{{ route("ContractHeader.activation",$contract_header->id) }}"
                                                  method="POST" v-on:submit="submit_form">
                                                @csrf
                                                <button type="submit" form="activation-form-{{ $contract_header->id }}"
                                                        class="dropdown-item">
                                            ...
                                            @can("edit", "ContractHeader")
                                                ...
                                                <a role="button"
                                                   href="{{ route("ContractHeader.edit",$contract_header->id) }}"
                                                   class="dropdown-item">
                                            ...
                                            @can("delete","ContractHeader")
                                                <div class="dropdown-divider"></div>
                                                <form class="w-100" id="delete-form-{{ $contract_header->id }}"
                                                      action="{{ route("ContractHeader.destroy",$contract_header->id) }}"
                                                      method="POST" v-on:submit="submit_form">
                                                    @csrf
                                                    @method("Delete")
                                                    <button type="submit" form="delete-form-{{ $contract_header->id }}"
                                                            class="dropdown-item">
...
@endsection
@section('modals')
    <div class="modal fade rtl" id="new_contract_modal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="main_submit_form" class="p-3" action="{{ route("ContractHeader.store") }}" method="POST"
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group mb-3 col-12">
                                ...
                                <input class="form-control text-center iranyekan @error('priority') is-invalid @enderror"
                                       type="text" name="priority"
                                       value="{{ old("priority") ? old("priority") : "0" }}">
                                @error('priority')
                                <span class="invalid-feedback iransans small_font" role="alert">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group mb-3 col-12">
                                ...
                                <input class="form-control text-center iranyekan @error('number') is-invalid @enderror"
                            ...
                            <div class="form-group mb-3 col-12">
                                ...
                                <input class="form-control text-center iranyekan @error('start_date') is-invalid @enderror persian_datepicker_range_from date_masked"
                                       data-mask="0000/00/00" type="text" name="start_date"
                            ...
                            <div class="form-group mb-3 col-12">
                                ...
                                <input class="form-control text-center iranyekan @error('end_date') is-invalid @enderror persian_datepicker_range_to date_masked"
                                       data-mask="0000/00/00" type="text" name="end_date" value="{{ old("end_date") }}">
                                @error('end_date')
                                <span class="invalid-feedback iransans small_font" role="alert">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group mb-3 col-12">
                                <label class="form-label iransans">مستندات قرارداد</label>
                                <m-file-browser :accept="['png','jpg','bmp','tiff','pdf','xlsx','txt']"
                                                :size="325000"></m-file-browser>
                            </div>
                        </div>
                    </form>
                </div>
